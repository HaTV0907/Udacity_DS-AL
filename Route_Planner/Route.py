...
def create_fScore(self):
    ...
    for i in self.map.intersections.keys():
        fScore[i]=math.inf
    fScore[0]= self.gScore[self.start] + heuristic_cost_estimate(self, self.start)
    return fScore
...
def get_current_node(self):
    ...
    for node in self.openSet:
        fScore[node] = self.calculate_fscore(node)
...
def distance(self, node_1, node_2):
    ...
    EL2D = math.sqrt(pow(x2 - x1, 2) + pow(y2 - y1, 2))
    return EL2D
...
def get_tentative_gScore(self, current, neighbor):
    ...
    tgS = self.get_gScore(current) + self.distance(current, neighbor)
    return tgS
...
def calculate_fscore(self, node):
    ...
    # REMEMBER F = G + H
    G = get_gScore(self, node)
    H = heuristic_cost_estimate(self, node)
    return G + H
...
def shortest_path(M,start,goal):
    planner = PathPlanner(M, start, goal)
    print("shortest path called" + str(planner.path))
    return planner.path